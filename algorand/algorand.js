...
var templates = [importTemplates.template1, importTemplates.template2, importTemplates.template3];
...
async function fundAccounts() {
        ...
        for (i = 0; i < accLimit; i++) {
            ...
            setInterval( function(){
                createTxs(i, masterAccount.addr, accounts[0], accounts[1], accounts[2]).catch()
            }, 12000); //update unique item every 12 seconds
...
async function createTxs(shipmentStage, masteraddr, account1, account2, account3) {
    ...
    for (i=0; i<accLimit; i++){
        ...
        //sign the transaction
        let signedTxn = algosdk.signTransaction(txn, accounts[i].sk);
        //submit the transaction
        let tx = (await algodclient.sendRawTransaction(signedTxn.blob));
        console.log("Item" + i + "transaction: " + tx.txId )
    }
}

//process 3 items every 60 seconds
fundAccounts().catch(); //initial call
setInterval(function() { fundAccounts().catch() }, 60000);